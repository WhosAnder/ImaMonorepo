...
const SignatureBox = ({ label, name, signature }: { label: string, name?: string, signature?: string | null }) => (
    <View style={styles.col}>
        ...
        <View style={[styles.sectionContent, { minHeight: 60, justifyContent: 'space-between' }]}>
            <Text style={{ fontSize: 8, textAlign: 'center', color: primaryColor, overflow: 'hidden', maxLines: 1 }}>
                {name || "—"}
            </Text>
            <View style={{ borderTopWidth: 1, borderTopColor: '#d1d5db', borderStyle: 'dashed', height: 30, justifyContent: 'flex-end', alignItems: 'center' }}>
                {signature ? (
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.header}>
                        ...
                        <View style={styles.headerTextContainer}>
                            <Text style={styles.headerTitle}>Formato de almacén proyecto</Text>
                    ...
                    <View style={styles.section}>
                        <View style={styles.row}>
                            ...
                            <LabeledPill label="FECHA Y HORA DE ENTREGA" value={formatDate(fechaHoraEntrega)} />
                        </View>
                        <View style={styles.row}>
                            <LabeledPill label="TURNO" value={turno} />
                            <LabeledPill label="NOMBRE ALMACENISTA" value={nombreAlmacenista} />
                        </View>
                        <View style={styles.row}>
                            <LabeledPill label="NOMBRE QUIEN RECIBE" value={nombreQuienRecibe} />
                            <View style={styles.col} />
                        </View>
                    </View>
                    ...
                    {/* Herramientas */}
                    <View style={styles.section}>
                        <Text style={styles.sectionHeader}>HERRAMIENTAS QUE SE ENTREGAN</Text>
                        <View style={styles.sectionContent}>
                            {herramientas && herramientas.length > 0 ? (
                                toolsOrPartsList(herramientas)
                            ) : (
                                <Text style={styles.emptyText}>Sin herramientas registradas</Text>
                            )}
                        </View>
                    </View>
                    ...
                    {/* Refacciones */}
                    <View style={styles.section}>
                        <Text style={styles.sectionHeader}>REFACCIONES QUE SE ENTREGAN</Text>
                        <View style={styles.sectionContent}>
                            {refacciones && refacciones.length > 0 ? (
                                toolsOrPartsList(refacciones)
                            ) : (
                                <Text style={styles.emptyText}>Sin refacciones registradas</Text>
                            )}
                        </View>
                    </View>
                    ...
                    <View style={[styles.section, { marginTop: 'auto', marginBottom: 20 }]}>
                        <View style={[styles.row, { marginBottom: 15 }]}>
                            <LabeledPill label="FECHA Y HORA DE RECEPCIÓN" value={formatDate(fechaHoraRecepcion)} />
                            <View style={styles.col} />
                        </View>
                        <View style={styles.row}>
                            <SignatureBox label="QUIEN ENTREGA" name={nombreQuienEntrega} signature={firmaQuienEntrega} />
                            <SignatureBox label="ALMACENISTA" name={nombreAlmacenista} signature={firmaAlmacenista} />
                            <SignatureBox label="QUIEN RECIBE" name={nombreQuienRecibe} signature={firmaQuienRecibe} />
                        </View>
                    </View>
                </View>
            </Page>
...
const toolsOrPartsList = (items: any[]) => items.map((item, idx) => (
    <View key={item.id || idx} style={styles.itemRow}>
        <View style={styles.itemHeader}>
            <Text style={styles.itemName}>{item.name}</Text>
            <Text style={styles.itemUnits}>{item.units} un.</Text>
        </View>
        <Text style={styles.itemObs}>{item.observations || 'Sin observaciones'}</Text>
        <Text style={styles.itemEvidences}>Evidencias: {item.evidences?.length || 0} / 5 fotos</Text>
    </View>
));
