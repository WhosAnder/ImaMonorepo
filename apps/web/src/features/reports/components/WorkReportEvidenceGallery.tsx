...
interface WorkReportEvidenceGalleryProps {
...
export const WorkReportEvidenceGallery: React.FC<
  WorkReportEvidenceGalleryProps
> = ({ actividadesRealizadas }) => {
  ...
  useEffect(() => {
    ...
    const initialState: ResolvedEvidence[] = allEvidences.map((ev) => ({
      id: ev.id || ev.key || String(Math.random()),
      url: null,
      isLoading: true,
      hasFailed: false,
    }));
    ...
    allEvidences.forEach(async (evidence, index) => {
      try {
        ...
        setResolvedEvidences((prev) => {
          const updated = [...prev];
          updated[index] = {
            id: evidence.id || evidence.key || String(Math.random()),
            url,
            isLoading: false,
            hasFailed: url === null,
          };
